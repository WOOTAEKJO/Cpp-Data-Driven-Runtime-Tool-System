		...
		if (ImGuiFileDialog::Instance()->Display("LoadEffect"))
		{
			if (ImGuiFileDialog::Instance()->IsOk())
			{
				if (m_iPickIndex_Object != -1)
				{
					...
					/*if (FAILED(m_pGameInstance->Add_Prototype(TEXT("ToolTrailBuffer_") + to_wstring(m_iNumLoad),
						CTrail_Buffer::Create(m_pDevice, m_pContext, filePathName))))
						RETURN_EFAIL;*/
...
#pragma  endregion
	...
	if (m_pTrailBufferEffect != nullptr)
	{
		...
		//m_pImgui_Manger->InputFloat4("Color", &m_TrailBufferDesc.vSolid_Color);
		ImGuiColorEditFlags flags;

		flags = ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_PickerHueBar |
			ImGuiColorEditFlags_NoInputs;

		//ImGui::ColorPicker4("InitColor", (float*)&m_TrailBufferDesc.vSolid_Color, flags, NULL);
...
{
	//Safe_Release(m_pTrailBufferEffect);
