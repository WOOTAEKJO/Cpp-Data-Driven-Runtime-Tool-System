...
HRESULT CImgui_Tab_ParticleEdit::Render()
{
	...
	if (!m_bModify)
	{
		...
		if (ImGuiFileDialog::Instance()->Display("LoadEffect"))
		{
			if (ImGuiFileDialog::Instance()->IsOk())
			{
				...
				/*if (FAILED(m_pGameInstance->Add_Prototype(TEXT("ToolParticle_") + to_wstring(m_iNumLoad),
					CParticle::Create(m_pDevice, m_pContext, filePathName))))
					RETURN_EFAIL;*/
				...
				/*m_pParticle = CObjPool_Manager::GetInstance()->Create_Object<CParticle>(TEXT("ToolParticle_") + to_wstring(m_iNumLoad),
					(LAYERTYPE)m_pGameInstance->Get_CreateLevelIndex());*/
	...
	if (m_pParticle != nullptr)
	{
		...
		{
			/*m_pParticle->Reset_Frame();
			m_pVIBufferCom->Reset_AllParticles();*/
		...
		else if (m_iControlType == 3) /* For. Color*/
		{
			...
			ImGui::Separator();
			//m_pImgui_Manger->InputFloat4("Min_InitColor", &m_InstancingDesc.vColor[0]);
			//m_pImgui_Manger->InputFloat4("Max_InitColor", &m_InstancingDesc.vColor[1]);
...
//void CImgui_Tab_ParticleEdit::ComboBox(const string& strTitle, const char** strList, _uint iListSize, _uint* iEasingType)
//{
...
//	}
//
//	/*if (ImGui::BeginCombo("SpriteType", strParticleSpriteTypes[m_tParticle_Desc.iSpriteType]))
//			{
//				for (int n = 0; n < IM_ARRAYSIZE(strParticleSpriteTypes); n++)
//				{
//					_bool bSelected = (m_tParticle_Desc.iSpriteType == n);
//					if (ImGui::Selectable(strParticleSpriteTypes[n], bSelected))
//					{
//						m_tParticle_Desc.iSpriteType = n;
//					}
//
//					if (bSelected)
//						ImGui::SetItemDefaultFocus();
