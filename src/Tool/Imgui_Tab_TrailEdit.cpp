	...
	if (m_pTrailEffect != nullptr)
	{
		...
		else if (m_TrailDesc.iShaderPassType == 3)
		{
			...
			if (m_TrailDesc.bDissolveParticle)
			{
				...
				{
					/*if (Compare_Wstr(m_TrailDesc.strDissolveParticleTag, L""))
						strDisplay = u8"선택해 주세요";
					else
						strDisplay = WstrToStr(m_TrailDesc.strDissolveParticleTag);*/
					strDisplay = u8"선택해 주세요";
				}
				else
		...
		ImGui::Separator();
		//ImGui::Checkbox("isClamp", &m_TrailDesc.bClamp);
		...
		//if (ImGui::Button("CurRotApply"))
		//{// IMGUI와 연계

		//	m_TrailDesc.vRotation = Get_RotEulerFromMatrix(m_pTrailEffect->Get_TransformCom().lock()->Get_WorldFloat4x4());
		//}
		...
		if (m_TrailDesc.bRotation)
		{
			...
			if (m_TrailDesc.bCurAxisUse)
			{
				...
				//m_pImgui_Manger->InputFloat("AxisAngle", &m_TrailDesc.fCurAxisAngle);
		...
		//m_TrailDesc.vColor = _float4(fTemp[0], fTemp[1], fTemp[2], fTemp[3]);
		/*m_pImgui_Manger->InputFloat4("InitColor", &m_TrailDesc.vColor);
		m_TrailDesc.vColor.x = Max(m_TrailDesc.vColor.x, m_TrailDesc.vColor.x);
		m_TrailDesc.vColor.y = Max(m_TrailDesc.vColor.y, m_TrailDesc.vColor.y);
		m_TrailDesc.vColor.z = Max(m_TrailDesc.vColor.z, m_TrailDesc.vColor.z);
		m_TrailDesc.vColor.w = Max(m_TrailDesc.vColor.w, m_TrailDesc.vColor.w);*/
