...
HRESULT CTrail_Effect::Render()
{
	...
	{
		//m_pModelCom->Bind_MeshMaterialToShader(iter,TEXTURETYPE::TextureType_DIFFUSE ,"g_DiffuseTexture");
...
HRESULT CTrail_Effect::Bind_ShaderResources()
{
	...
	// For.Distortion
	/*if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_vDistortionValue", &m_TrailDesc.vDistortionValue, sizeof(_float3)))) RETURN_EFAIL;
	if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_fDistortionScale", &m_TrailDesc.fDistortionScale, sizeof(_float)))) RETURN_EFAIL;
	if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_fDistortionBias", &m_TrailDesc.fDistortionBias, sizeof(_float)))) RETURN_EFAIL;
	if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_fDistortionWeight", &m_TrailDesc.fNoiseWeight, sizeof(_float)))) RETURN_EFAIL;*/
	...
	if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_bMaskInverse", &m_TrailDesc.bMaskInverse, sizeof(m_TrailDesc.bMaskInverse))))
		RETURN_EFAIL;
	/*if (FAILED(m_pModelCom->ShaderComp()->Bind_RawValue("g_bClamp", &m_TrailDesc.bClamp, sizeof(m_TrailDesc.bClamp))))
		RETURN_EFAIL;*/
